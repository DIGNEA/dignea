...
template<typename T>
class is_string_test : public testing::Test {
};
...
EXPECT_EQ(fmt::detail::compute_width(result),
4);
EXPECT_EQ(result
.
...
EXPECT_EQ(
        fmt::format(L"{first}{second}{first}{third}",
...
int v[3] = {1, 2, 3};
EXPECT_EQ(fmt::format(L"({})", fmt::join(v, v + 3, L", ")),
L"(1, 2, 3)");
auto t = std::tuple<wchar_t, int, float>('a', 1, 2.0f);
EXPECT_EQ(fmt::format(L"({})", fmt::join(t, L", ")),
L"(a, 1, 2)");
}
...
wchar_t format_str[] = {
        L'{', L':',
        '+' | static_cast<wchar_t>(1 << fmt::detail::num_bits<char>()), L'}',
        0};
...
namespace fake_qt {
    ...
    to_string_view(const QString &s) FMT_NOEXCEPT {
...
fmt::vformat(loc,
L"{:L}",
fmt::basic_format_args<wcontext>(as)
));
EXPECT_EQ(L"1234567",
...
FMT_BEGIN_NAMESPACE
        ...
        public:
            FMT_CONSTEXPR typename basic_format_parse_context<charT>::iterator
            parse(
            ...
            format(const std::complex<double> &c,
                ...
                return format_to(ctx.out(),
                                 runtime("{:" + fill_align_width + "}"),
                                 c.real() != 0 ? fmt::format("({}+{}i)", real,
                                                             imag)
                                               : fmt::format("{}i", imag));
            }
        };
...
EXPECT_EQ(s,
"(1+2i)");
EXPECT_EQ(fmt::format("{:.2f}", std::complex<double>(1, 2)),
"(1.00+2.00i)");
EXPECT_EQ(fmt::format("{:8}", std::complex<double>(1, 2)),
"  (1+2i)");
}
