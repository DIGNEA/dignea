...
FMT_BEGIN_NAMESPACE
        ...
        struct formatter<test_struct, Char> {
            ...
            auto
            format(test_struct, format_context &ctx) -> decltype(ctx.out()) {
...
struct check_custom {
    test_result operator()(
        ...
        auto ctx = fmt::format_context(
                fmt::detail::buffer_appender<char>(buffer),
                fmt::format_args());
...
struct test_format_string_handler {
    ...
    template<typename T>
    constexpr int on_arg_id(T) { return 0; }
...
FMT_BEGIN_NAMESPACE
        ...
        struct formatter<convertible_to_int> {
            ...
            auto format(convertible_to_int,
                        format_context &ctx) -> decltype(ctx.out()) {
...
EXPECT_EQ(to_string_view(test_string<char>("42")),
"42");
...
EXPECT_EQ("100000000", fmt::format("{:x}",
