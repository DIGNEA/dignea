...
auto runtime_named_field_compiled =
        fmt::detail::compile<decltype(fmt::arg("arg", 42))>(
                FMT_COMPILE("{arg}"));
...
FMT_BEGIN_NAMESPACE
        ...
        struct formatter<to_stringable> {
            ...
            template<typename FormatContext>
            auto format(const to_stringable &,
...
EXPECT_EQ("42 42",
test_format<6>(FMT_COMPILE("{} {}"), uint32_t{
42}, uint64_t{
42}));
...
EXPECT_EQ("foo", test_format<4>(FMT_COMPILE("{}"),

test_formattable()

));
EXPECT_EQ("bar", test_format<4>(FMT_COMPILE("{:b}"),
