...
void MiW::run() {
    if (this->problem) {
        if (auto pKP = dynamic_pointer_cast<KPNR>(this->problem)) {
            ...
            auto totalProfit = 0;
            auto inKP = 0;
            // Needs to sort the variables based on the weights vector
            ...
            throw std::runtime_error(
                "MiW Heuristic must be used with a KP instance");
        }
    } else {
        throw std::runtime_error("Problem is not set in MiW::run");
    }
}
