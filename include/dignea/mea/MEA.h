 ...
 * (MEA). This algorithm uses a Weighted Fitness Function (WFF) to evaluate how
...
class MEA : public AbstractEA<IS> {
   public:
    using Alg = AbstractEA<OS>; /*!< Acronym for an AbstractEA<OS> */

    /**
     * @brief Construct a new MEA object using a fitness ratio of 0.6 and
     * novelty of 0.4 by default. Use the MEABuilder instead of the constructor
     * for safe experimentation.
     *
     * @param fitness Ratio of fitness for the WFF
     * @param novelty Ratio of novelty for the WFF
     */
    MEA(const float &fitness = 0.6f, const float &novelty = 0.4f);
    ...
    /// @returns a string with the content MEA
    [[nodiscard]] string getName() const override { return "MEA"; };
    ...
    /**
     * @brief Get the mutation rate used in MEA for mutation instances during
    ...
    /// @brief Set the mutation rate used in MEA for mutation instances during
    /// the evolutionary process
    /// @param mRate
    void setMutationRate(float mRate) { this->mutationRate = mRate; }

    /**
     * @brief Get the crossover rate used in MEA for applying the crossover
    ...
    /// @brief Set the crossover rate used in MEA for applying the crossover
    /// operator to instances during the evolutionary process
    /// @param cRate
     ...
     * must perform over each instance at every generation of the MEA.
     *
     * @return int
     */
    ...
    /// MEA. Due to stochastic nature of most algorithms used in MEA the number
    ...
    /// @brief Get the mutation operator used to mutate instances in MEA
    ...
    /// @brief Get the crossover operator used in MEA.
    ...
    /// @brief Get the number of generations that MEA must perform.
    /// @return
    inline int getGenerations() { return this->getMaxEvaluations(); };

    /// @brief Get the Novelty Search approach used in MEA.
    /// @return
...
/**
 * @brief Creates a default MEA with only the evaluation as EasyInstances
 * @tparam IP Type of Instance Problem to toSolve
 * @tparam IS Type of instances of the Optimisation Problem
 * @tparam OP Optimisation problem to toSolve with the configurations
 * @tparam OS Solutions to the OP type
 */
template <typename IP, typename IS, typename OP, typename OS>
MEA<IP, IS, OP, OS>::MEA(const float &fitness, const float &novelty)
    : AbstractEA<IS>(),
      mutationRate(0.05),
      crossoverRate(0.8),
      repetitions(10),
...
/**
 * @brief Interface to run the MEA for generating a set of instances for the
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
void MEA<IP, IS, OP, OS>::run() {
    if (instProb) {
#ifdef DEBUG
        std::cout << "Running MEA" << std::endl;
#endif
        ...
        throw std::logic_error("MEAProblem not set in MEA::run");
    }
}
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
void MEA<IP, IS, OP, OS>::updateEvolution(vector<IS> &solutions) {
...
/**
 * @brief Evaluation phase of the MEA
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
void MEA<IP, IS, OP, OS>::evaluationPhase(vector<IS> &individuals) {
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
Front<IS> MEA<IP, IS, OP, OS>::getResults() const {
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
void MEA<IP, IS, OP, OS>::initProgress() {
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
void MEA<IP, IS, OP, OS>::updateProgress() {
    this->performedEvaluations++;
}

/**
 * @brief Finishes the execution of the MEA
 * @tparam IP Type of Instance Problem to toSolve
 * @tparam IS Type of instances of the Optimisation Problem
 * @tparam OP Optimisation problem to toSolve with the configurations
 * @tparam OS Solutions to the OP type
 */
template <typename IP, typename IS, typename OP, typename OS>
void MEA<IP, IS, OP, OS>::finishProgress() {
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
bool MEA<IP, IS, OP, OS>::isStoppingConditionReached() {
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
void MEA<IP, IS, OP, OS>::createInitialPopulation() {
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
void MEA<IP, IS, OP, OS>::evaluatePopulation(vector<IS> &individuals) {
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
[[maybe_unused]] void MEA<IP, IS, OP, OS>::evaluateIndividual(IS &individual) {
    // Creates the optimisation problem for the EAs
    shared_ptr<OP> optProblem = instProb->genOptProblem(individual);
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
void MEA<IP, IS, OP, OS>::reproduction(IS &solution1, IS &solution2) {
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
void MEA<IP, IS, OP, OS>::replacement(vector<IS> &individuals) {
    if (individuals.size() != this->population.size()) {
        std::string where = " replacement at MEA";
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
void MEA<IP, IS, OP, OS>::setPortfolio(vector<unique_ptr<Alg>> &configs) {
    algPortfolio.clear();
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
void MEA<IP, IS, OP, OS>::setInstanceProblem(unique_ptr<IP> problem) {
    this->instProb = move(problem);
}

/**
 * @brief Returns the portfolio used by the MEA
 * @tparam IP Type of Instance Problem to toSolve
 * @tparam IS Type of instances of the Optimisation Problem
 * @tparam OP Optimisation problem to toSolve with the configurations
 * @tparam OS Solutions to the OP type
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
vector<AbstractEA<OS> *> MEA<IP, IS, OP, OS>::getPortfolio() const {
...
/**
 * @brief Creates the JSON representation of the MEA
 * @tparam IP Type of Instance Problem to toSolve
 * @tparam IS Type of instances of the Optimisation Problem
 * @tparam OP Optimisation problem to toSolve with the configurations
 * @tparam OS Solutions to the OP type
 ...
 */
template <typename IP, typename IS, typename OP, typename OS>
json MEA<IP, IS, OP, OS>::to_json() const {
    json data;
