...
auto check_move_buffer =
        [](const char *str,
           basic_memory_buffer<char, 5, std_allocator> &buffer) {
            std::allocator<char> *alloc = buffer.get_allocator().get();
            basic_memory_buffer<char, 5, std_allocator> buffer2(
                    std::move(buffer));
...
struct fmt::formatter<explicitly_convertible_to_std_string_view>
    ...
    auto
    format(explicitly_convertible_to_std_string_view v, format_context &ctx)
    -> decltype(ctx.out()) {
...
FMT_BEGIN_NAMESPACE
        template<>
        struct formatter<converible_to_anything> {
            FMT_CONSTEXPR auto
...
FMT_BEGIN_NAMESPACE
        ...
        struct formatter<date> {
            ...
            auto
            format(const date &d, format_context &ctx) -> decltype(ctx.out()) {
...
EXPECT_SYSTEM_ERROR(
        {
            ...
            if (!f)
                throw fmt::system_error(errno, "Cannot open file '{}'",
                                        filename);
            fclose(f);
        },
...
EXPECT_WRITE(
        stdout, fmt::print("{}", std::numeric_limits<double>::infinity()),
        "inf");
}
...
FMT_BEGIN_NAMESPACE
        ...
        struct formatter<variant> : dynamic_formatter<> {
            auto
            format(variant value, format_context &ctx) -> decltype(ctx.out()) {
...
fmt::to_string (adl_test::fmt::detail::foo())

);

enum test_enum2 : unsigned char {
    test_value
};
EXPECT_EQ("0",
fmt::to_string(test_value)
);
}
...
FMT_BEGIN_NAMESPACE
        ...
        struct formatter<check_back_appender> {
            ...
            auto
            format(check_back_appender, Context &ctx) -> decltype(ctx.out()) {
                auto out = ctx.out();
                static_assert(
                        std::is_same<decltype(++out), decltype(out) &>::value,
