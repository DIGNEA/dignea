...
#define MAXSTATES \
    600000  // The size here will limit the speed of the algorithm, but the
            // algorithm will fail for some problems if maxstates is too large,
            // this must be considered when running
...
void pop(allinfo *a, int side, item **f, item **l) {
    ...
    switch (side) {
        case LEFT:
            if (a->intv1 == a->intv1b) errorx("pop left");
            (a->intv1)--;
            pos = a->intv1;
            break;
        case RIGHT:
            if (a->intv2 == a->intv2b) errorx("pop right");
            (a->intv2)++;
            pos = a->intv2;
            break;
    }
...
void reduceset(allinfo *a) {
    ...
    if (a->s < a->fsort) {
        if (a->intv1 == a->intv1b) {
            ...
            ps = f->p;
            ws = f->w; /* default: pick first item */
    ...
    if (a->t > a->lsort) {
        if (a->intv2 == a->intv2b) {
            ...
            pt = l->p;
            wt = l->w; /* default: pick first item */
...
void copyproblem(item *f, item *l, int *p, int *w, int *x) {
    ...
    for (i = f, m = l + 1, pp = p, ww = w, xx = x; i != m;
         i++, pp++, ww++, xx++) {
        i->p = *pp;
        i->w = *ww;
        i->x = (BOOL *)xx;
...
void findbreak(allinfo *a) {
    ...
    i--;
    psum -= i->p;
    wsum -= i->w; /* we went one item too far */
...
double minknap_wrap(int n, int *p, int *w, int *x, int c) {
    int z;
...
MinKnap::MinKnap() : Heuristic<BoolFloatSolution>() {
...
void MinKnap::run() {
    if (this->problem) {
        if (auto pKP = dynamic_pointer_cast<KPNR>(this->problem)) {
            ...
            auto elapsedTime = MinKnapSpace::minknap_wrap(
            ...
            throw std::runtime_error(
                "MinKnap Heuristic must be used with a KP instance");
        }
    } else {
        throw std::runtime_error("Problem is not set in MinKnap::run");
    }
}

void MinKnap::setProblem(shared_ptr<KP> prob) { this->problem = prob; }

void MinKnap::setProblem(KP *prob) { this->problem.reset(prob); }

Front<BoolFloatSolution> MinKnap::getResults() const {
