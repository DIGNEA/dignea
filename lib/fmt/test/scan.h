...
FMT_BEGIN_NAMESPACE
        ...
        namespace detail {
            ...
            struct custom_scan_arg {
                ...
                void (*scan)(void *arg, scan_parse_context &parse_ctx,
                             scan_context &ctx);
            };
            ...
            public:
                ...
                scan_arg(int &value) : type(scan_type::int_type),
                                       int_value(&value) {}

                scan_arg(unsigned &value) : type(scan_type::uint_type),
                                            uint_value(&value) {}

                scan_arg(long long &value)
                        : type(scan_type::long_long_type),
                          long_long_value(&value) {}

                scan_arg(unsigned long long &value)
                        : type(scan_type::ulong_long_type),
                          ulong_long_value(&value) {}

                scan_arg(std::string &value) : type(scan_type::string_type),
                                               string(&value) {}

                scan_arg(fmt::string_view &value)
                        : type(scan_type::string_view_type),
                          string_view(&value) {}
            ...
            private:
                template<typename T>
                static void
                scan_custom_arg(void *arg, scan_parse_context &parse_ctx,
                                scan_context &ctx) {
                    scanner<T> s;
        ...
        namespace detail {
            ...
            public:
                scan_handler(string_view format, string_view input,
                             scan_args args)
                        : parse_ctx_(format), scan_ctx_(input), args_(args),
                          next_arg_id_(0) {}
                ...
                void on_replacement_field(int, const char *) {
                    ...
                    switch (arg_.type) {
                        ...
                        case scan_type::string_type:
                            while (it != end && *it != ' ')
                                arg_.string->push_back(*it++);
                            scan_ctx_.advance_to(it);
                            break;
                        case scan_type::string_view_type: {
                            auto s = it;
                            while (it != end && *it != ' ') ++it;
                            *arg_.string_view = fmt::string_view(s, to_unsigned(
                                    it - s));
                            scan_ctx_.advance_to(it);
                            break;
                        }
                ...
                const char *
                on_format_specs(int, const char *begin, const char *) {
        ...
        std::array<detail::scan_arg, sizeof...(Args)>
        make_scan_args(Args &... args) {
            return {{args...}};
        }
