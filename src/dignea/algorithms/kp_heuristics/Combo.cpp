...
stype combo(item *f, item *l, stype c, stype lb, stype ub, boolean def,
            boolean relx);
...
void give_time(long *time) { /* return the number of milliseconds used */
    struct tms timeend;
    double t1;
    times(&timeend);
...
static void pfree(
    void *p) { /* semi-own free routine which makes additional test */
...
static void pop(allinfo *a, int side, item **f, item **l) {
    ...
    switch (side) {
        case LEFT:
            if (a->intv1 == a->intv1b) error("pop left");
            (a->intv1)--;
            pos = a->intv1;
            *f = pos->f;
            *l = pos->l;
            break;
        case RIGHT:
            if (a->intv2 == a->intv2b) error("pop right");
            (a->intv2)++;
            pos = a->intv2;
            *f = pos->f;
            *l = pos->l;
...
static void definesolution(allinfo *a) {
    ...
    if (a->fullsol) {
        for (i = a->fitem, m = a->litem + 1, h = a->ffull; i != m; i++, h++)
            *i = *h;
        return;
    }
...
static void partsort(allinfo *a, item *f, item *l, stype ws, stype c,
                     int what) {
...
static void sursort(item *f, item *l, itype sur, stype c, stype *p1, stype *w1,
                    item **b) {
    register itype s;
    register prod mp, mw;
    register item *i, *j, *m;
    ...
    for (;;) {
        ...
        for (;;) {
            do {
                ...
                ps += i->p;
                i++;
            } while (DET(i->p, i->w + s, mp, mw) > 0);
            do {
                j--;
            } while (DET(j->p, j->w + s, mp, mw) < 0);
            if (i > j) break;
            SWAP(i, j);
        }
        if (ws <= c) {
...
static boolean haschance(allinfo *a, item *i, int side) {
    ...
    w = a->b->w;
    if (side == LEFT) {
        return (DET(a->psumb - i->p - a->z - 1, a->wsumb - i->w - a->c, p, w) >=
                0);
    } else {
        return (DET(a->psumb + i->p - a->z - 1, a->wsumb + i->w - a->c, p, w) >=
                0);
...
static void surbin(item *f, item *l, itype s1, itype s2, stype c, stype dantzig,
                   ntype card, stype *sur, stype *u) {
    item *b;
...
static void reduceset(allinfo *a) {
    ...
    if (!atstart) {
        ...
        w = a->ws;
        for (i = rm, m = v; i != m; i--) {
            if (DET(i->psum - z, i->wsum - c, p, w) >= 0) {
                k--;
                *k = *i;
            }
        }
...
void findcore(allinfo *a) {
    ...
    if (a->fitem < s) {
        s--;
        swapout(a, a->fitem, LEFT);
    }
    if (a->litem > t) {
        t++;
        swapout(a, a->litem, RIGHT);
    }
...
extern stype combo(item *f, item *l, stype c, stype lb, stype ub, boolean def,
                   boolean relx)
/* f,l : first, last item                                               */
...
/* returns the objective value of the problem                           */
{
    ...
    while ((a.d.size > 0) && (a.z < a.ub)) {
        ...
        if ((!heur) && (a.d.size > MINHEUR)) {
            heuristic(&a);
            heur = TRUE;
        }

        /* find tight bound when needed */
        if ((!relx) && (a.d.size > MINSET)) {
            surrelax(&a);
            relx = TRUE;
        }

        /* use rudimentary divisibility to decrease c */
        if ((!rudi) && (a.d.size > MINRUDI)) {
            rudidiv(&a);
            rudi = TRUE;
        }
    }
...
Combo::Combo() : Heuristic<BoolFloatSolution>() {
    this->populationSize = 1;
    this->population.resize(this->populationSize);
}

stype maketest(KPNR *instance, item *f, item *l, int type, int r, int v) {
    register item *j;
    stype c;

    // Copy the DIGNEA instance into Pisingers code
    auto profits = instance->getProfits();
    auto weights = instance->getWeights();
    auto nItems = instance->getNumberOfVars();
    int counter = 0;
    for (j = f; j <= l; j++) {
        j->w = weights[counter];
        j->p = profits[counter];
...
void testinstance(KPNR *instance, item **f, item **l, int n, int r, int type,
                  int v) {
    item *a;
    auto nItems = instance->getNumberOfVars();
    /* allocate space for test example */
    // Creates a pointer for N items
    a = (item *)palloc(nItems, sizeof(item));
    *f = a;
    *l = &a[nItems - 1];

    /* make test instance */
    maketest(instance, *f, *l, type, r, v);
}
...
void Combo::run() {
    if (this->problem) {
        if (auto pKP = dynamic_pointer_cast<KPNR>(this->problem)) {
            ...
            stype z;
            int n, r, type, v;
            // Create KP instance with the information from pKP
            testinstance(pKP.get(), &f, &l, n, r, type, v);
            auto c = pKP->getCapacity();
            z = combo(f, l, c, 0, std::numeric_limits<stype>::max(), false,
                      false);

            // Building DIGNEA solution
            this->endTime = chrono::high_resolution_clock::now();

            auto elapsedTime = std::chrono::duration<double, std::milli>(
            ...
            throw std::runtime_error(
                "Combo Heuristic must be used with a KP instance");
        }
    } else {
        throw std::runtime_error("Problem is not set in Combo::run");
    }
}

void Combo::setProblem(shared_ptr<KP> prob) { this->problem = prob; }

void Combo::setProblem(KP *prob) { this->problem.reset(prob); }

Front<BoolFloatSolution> Combo::getResults() const {
    return Front<BoolFloatSolution>(this->population);

}  // namespace comboHeuristic